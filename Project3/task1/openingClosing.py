...
for i in range(imageH):
    for j in range(imageW):
        ...
        kernelValue = calculateErosionat(currentWidthOffset,currentHeightOffset, a)
...
for i in range(updatedImageDataH):
    for j in range(updatedImageDataW):
        currentHeightOffset = i
        currentWidthOffset = j
        kernelValue2 = calculateDilationat(currentWidthOffset,currentHeightOffset, b)
...
for i in range(newUpdatedImageDataH):
    for j in range(newUpdatedImageDataW):
        currentHeightOffset = i
        currentWidthOffset = j
        kernelValue3=calculateDilationat(currentWidthOffset,currentHeightOffset, c)
...
for i in range(updatedImageDataIIH):
    for j in range(updatedImageDataIIW):
        currentHeightOffset = i
        currentWidthOffset = j
        kernelValue4=calculateErosionat(currentWidthOffset,currentHeightOffset, d, order=1)
...
openingClosing=np.asarray(newUpdatedImageDataII,dtype='uint8')
cv2.imwrite('/Users/stutishukla/Downloads/Project3_images/task1/res_noise2.jpg',openingClosing)
...
openingClosing=openingClosing-finalImage
#boundaryExtraction=np.asarray(newUpdatedImageDataII)
cv2.imwrite('/Users/stutishukla/Downloads/Project3_images/task1/res_bound2.jpg',openingClosing)

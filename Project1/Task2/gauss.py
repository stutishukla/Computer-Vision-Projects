...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct  5 18:05:18 2018
    ...
    for i in range(kernalHeight):
        for j in range(kernalWidth):
            ...
            if ((currentWidthIndex < 0) or (currentWidthIndex >= imageWidth)):
                continue
            result += (img[int(currentHeightIndex)][int(currentWidthIndex)] * gauss_kernel[int(i)][int(j)])
    return int(result)


def fxy(x, y, sigma):
    temp1 = float(1 /float(2 * math.pi * sigma * sigma))
    temp2 = np.exp(-((float(x * x + y * y))/(2 * sigma * sigma)))
    return temp1 * temp2
...
outputImage1 = applyGuassianKernaltoImage(img, gauss_kernel)  # applying computed gaussian kernal to our image
# print outputImage here
b1 = np.asarray(outputImage1)
cv2.imwrite('/Users/stutishukla/Downloads/Result/octave1/task2-1.png', b1)

sigmaValue2 = 1
# for sigmaValue in sigmaValues: # Iterating for each sigma value
gauss_kernel_raw, total = getGuassianKernalforSigma(sigmaValue2)  # computing gaussian kernal for given sigma value
gauss_kernel = normalizeGaussKernel(gauss_kernel_raw, total)
outputImage2 = applyGuassianKernaltoImage(img, gauss_kernel)  # applying computed gaussian kernal to our image
# print outputImage here
b2 = np.asarray(outputImage2)
cv2.imwrite('/Users/stutishukla/Downloads/Result/octave1/task2-2.png', b2)

DogI = DiffOfGauss(b1, b2)
DogIA = normalizeDOG(DogI)
DogInorm = np.asarray(DogIA)
cv2.imwrite('/Users/stutishukla/Downloads/Result/octave1/task2-1DOG.png', DogInorm)

sigmaValue3 = (math.sqrt(2))
# for sigmaValue in sigmaValues: # Iterating for each sigma value
gauss_kernel_raw, total = getGuassianKernalforSigma(sigmaValue3)  # computing gaussian kernal for given sigma value
gauss_kernel = normalizeGaussKernel(gauss_kernel_raw, total)
outputImage3 = applyGuassianKernaltoImage(img, gauss_kernel)  # applying computed gaussian kernal to our image
# print outputImage here
b3 = np.asarray(outputImage3)
cv2.imwrite('/Users/stutishukla/Downloads/Result/octave1/task2-3.png', b3)

DogII = DiffOfGauss(b2, b3)
DogIIA = normalizeDOG(DogII)
DogInorm = np.asarray(DogIIA)
cv2.imwrite('/Users/stutishukla/Downloads/Result/octave1/task2-2DOG.png', DogInorm)

sigmaValue4 = 2
# for sigmaValue in sigmaValues: # Iterating for each sigma value
gauss_kernel_raw, total = getGuassianKernalforSigma(sigmaValue4)  # computing gaussian kernal for given sigma value
gauss_kernel = normalizeGaussKernel(gauss_kernel_raw, total)
outputImage4 = applyGuassianKernaltoImage(img, gauss_kernel)  # applying computed gaussian kernal to our image
# print outputImage here
b4 = np.asarray(outputImage4)
cv2.imwrite('/Users/stutishukla/Downloads/Result/octave1/task2-4.png', b4)

DogIII = DiffOfGauss(b3, b4)
DogIIIA = normalizeDOG(DogIII)
DogInorm = np.asarray(DogIIIA)
cv2.imwrite('/Users/stutishukla/Downloads/Result/octave1/task2-3DOG.png', DogInorm)

sigmaValue5 = 2 * (math.sqrt(2))
# for sigmaValue in sigmaValues: # Iterating for each sigma value
gauss_kernel_raw, total = getGuassianKernalforSigma(sigmaValue5)  # computing gaussian kernal for given sigma value
gauss_kernel = normalizeGaussKernel(gauss_kernel_raw, total)
outputImage5 = applyGuassianKernaltoImage(img, gauss_kernel)  # applying computed gaussian kernal to our image
# print outputImage here
b5 = np.asarray(outputImage5)
cv2.imwrite('/Users/stutishukla/Downloads/Result/octave1/task2-5.png', b5)

DogIV = DiffOfGauss(b4, b5)
DogIVA = normalizeDOG(DogIV)
DogInorm = np.asarray(DogIVA)
cv2.imwrite('/Users/stutishukla/Downloads/Result/octave1/task2-4DOG.png', DogInorm)


#finalMatHeight,finalMatWidth = getDimensions(DogIIA)
finaMatrix = []
...
for keypoint in finaMatrix:
    cv2.circle(img,(keypoint[1],keypoint[0]),1,(255,255,0), -1)
keypointDetectMax=np.asarray(img)
cv2.imwrite('/Users/stutishukla/Downloads/Result/task2/keyPointDetection/OctaveI/keyPointDetectMaxDOGII.png', img)

keypointMinimumDetection(DogIIA, DogIA, DogIIIA,finaMatrix)
#keypointMinimumDetection(DogIIA, DogIA, DogIIIA,finaMatrix)
for keypoint in finaMatrix:
    cv2.circle(img,(keypoint[1],keypoint[0]),1,(255,255,0), -1)
cv2.imwrite('/Users/stutishukla/Downloads/Result/task2/keyPointDetection/OctaveI/keyPointDetectMinDOGII.png', img)
...
for keypoint in finaMatrix:
    cv2.circle(img,(keypoint[1],keypoint[0]),1,(255,255,0), -1)
keypointDetectMax=np.asarray(img)
cv2.imwrite('/Users/stutishukla/Downloads/Result/task2/keyPointDetection/OctaveI/keyPointDetectMaxDOGIII.png', img)

keypointMinimumDetection(DogIIIA, DogIIA, DogIVA,finaMatrix)
#keypointMinimumDetection(DogIIA, DogIA, DogIIIA,finaMatrix)
for keypoint in finaMatrix:
    cv2.circle(img,(keypoint[1],keypoint[0]),1,(255,255,0), -1)
cv2.imwrite('/Users/stutishukla/Downloads/Result/task2/keyPointDetection/OctaveI/keyPointDetectMinDOGIII.png', img)